...
export function AdminBannersList({ banners: initialBanners }: AdminBannersListProps) {
  ...
  const handleDelete = async (id: string, title: string) => {
    ...
    const res = await fetch(withBasePath(`/api/banners/${id}`), { method: "DELETE" });
    if (!res.ok) {
      alert("刪除失敗");
      return;
    }
  ...
  const handleSave = async (banner: Banner) => {
    ...
    const res = await fetch(withBasePath(exists ? `/api/banners/${banner.id}` : "/api/banners"), {
    ...
    const normalized: Banner = {
      ...
      backgroundImage: String(saved.backgroundImage ?? ""),
      primaryCta: {
        label: String(saved.primaryCtaLabel ?? ""),
        href: String(saved.primaryCtaHref ?? ""),
      },
      secondaryCta:
        saved.secondaryCtaLabel && saved.secondaryCtaHref
          ? { label: String(saved.secondaryCtaLabel), href: String(saved.secondaryCtaHref) }
          : undefined,
    ...
    let next: Banner[];
    if (exists) {
      next = banners.map((b) => (b.id === normalized.id ? normalized : b));
      setBanners(next);
      setEditingId(null);
    } else {
      next = [...banners, normalized].sort((a, b) => a.order - b.order);
      setBanners(next);
      setAdding(false);
    }

    // if set as default, clear local state on others
    if (normalized.isDefault) {
      next = next.map((b) => (b.id === normalized.id ? b : { ...b, isDefault: false }));
      setBanners(next);
    }
  ...
  const handleMove = async (id: string, direction: "up" | "down") => {
    ...
    setBanners(next);
    const res = await fetch(withBasePath("/api/banners/reorder"), {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ids: next.map((b) => b.id) }),
    });
    if (!res.ok) alert("排序儲存失敗（已在畫面更新，但未寫入資料庫）");
  };
